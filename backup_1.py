...
class Actor(tf.keras.Model):
    ...
    def save_model(self,path):
        #print(path)
        self.save(path, save_format="tf",include_optimizer=True)
        #print(f"Actor model saved at {path}")
...
class Critic(tf.keras.Model):
    ...
    def save_model(self,path):
        ...
        self.save(path, save_format="tf",include_optimizer=True)
        #print(f"Critic model saved at {path}")
...
class PPOAgent(tf.keras.Model):
    ...
    def get_action_and_log_prob(self, mean):
        std = tf.exp(self.log_std)  # Learnable log_std
    ...
    def save(self):
        ...
        model_dir = f'PPO'
        model_dir_actor = f'PPO/Actor'
        model_dir_critic = f'PPO/Critic'
       
       
      
        for directory in [model_dir, model_dir_actor, model_dir_critic]:
            if not os.path.exists(directory):
                os.makedirs(directory)

        x = len(os.listdir(model_dir_actor))

        model_file_nums = f'policy_{x}'

        self.actor.compile(optimizer=self.optimizer, loss=self.loss)
        self.critic.compile(optimizer=self.optimizer, loss=self.loss)

        self.actor.save_weights(os.path.join(model_dir, f'Actor/{model_file_nums}_weights.h5'))
        self.critic.save_weights(os.path.join(model_dir, f'Critic/{model_file_nums}_weights.h5'))
        ...
        print(f"Model weights are saved as {model_file_nums} saved at {model_dir}")


    def chkpt_save(self,episode,timestep,cumulative_score,action_std_init,flag):

        checkpoint_dir = f'checkpoints'
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)

        chkt_file_nums = len(os.listdir(checkpoint_dir))
        if chkt_file_nums != 0:
            chkt_file_nums -= 1
        ...
        if flag == 1:
            ...
            chkt_file_nums = chkt_file_nums

        chkpt_file = os.path.join(checkpoint_dir, f'checkpoint_ppo_{chkt_file_nums}.pickle')
        ...
        with open(chkpt_file, 'wb') as f:
            pickle.dump(data, f)


        print(f"Checkpoint saved at {chkpt_file}")
        print()
    ...
    def load(self):
        ...
        x = len([d for d in os.listdir(model_dir_actor) if os.path.isdir(os.path.join(model_dir_actor, d))])
        model_file_nums = f'policy_{x}'

        dummy_input = tf.random.normal((1, self.obs_dim))

        _ = self.actor(dummy_input)
        _ = self.old_actor(dummy_input)
        _ = self.critic(dummy_input)
        _ = self.old_critic(dummy_input)

        self.actor.load_weights(os.path.join(model_dir, f'Actor/{model_file_nums}_weights.h5'))
        self.critic.load_weights(os.path.join(model_dir, f'Critic/{model_file_nums}_weights.h5'))
        ...
        print(f"Model weights are loaded from {model_file_nums}")


    def chkpt_load(self):

        checkpoint_dir = f'checkpoints'
        chkt_file_nums = len(os.listdir(checkpoint_dir))-2

        checkpoint_file = os.path.join(checkpoint_dir ,f'checkpoint_ppo_{chkt_file_nums}.pickle')
        ...
        action_std_init = checkpoint_data['action_std_init']
        
        print(f"Checkpoint loaded from {checkpoint_file}")
        #print(f"Resuming from Episode: {episode}, Timestep: {timestep}, Cumulative Score: {cumulative_score}")
...
def train():
    ...
    while timestep < TOTAL_TIMESTEPS:
        ...
        if episode %10 == 0:
            agent.learn()
            agent.chkpt_save(episode,timestep,cumulative_score,ACTION_STD_INIT,0)

        if episode%100 == 0:
            agent.save()
            agent.chkpt_save(episode,timestep,cumulative_score,ACTION_STD_INIT,1)
