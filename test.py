...
class PPOAgent(tf.keras.Model):
    ...
    def get_action_and_log_prob(self, mean):
        ...
        dist = tfd.MultivariateNormalDiag(loc=mean, scale_diag=2*self.log_std)
        action = dist.sample()
    ...
    def learn(self):
        ...
        for i in range(self.n_updates_per_iteration):
            with tf.GradientTape() as tape_a, tf.GradientTape() as tape_c:
                ...
                #critic_loss = 0.5 * tf.clip_by_value(self.loss(values, returns), -1.0, 1.0)
            ...
            if any(tf.reduce_any(tf.math.is_nan(g)) for g in grads_a + grads_c):
            ...
            print(f"Update {i+1}/{self.n_updates_per_iteration}: Actor Loss = {actor_loss.numpy():.6f}, "
                f"Critic Loss = {critic_loss.numpy():.6f}, Entropy = {tf.reduce_mean(dist_entropy).numpy():.6f}")
