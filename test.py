...
class PPOAgent(tf.keras.Model):
    ...
    def __init__(self, name="PPOAgent", **kwargs):
        ...
        #self.log_std = tf.Variable(tf.fill((self.action_dim,), tf.math.log(self.action_std_init)), trainable=True, dtype=tf.float32)
    ...
    def learn(self):
        ...
        for i in range(self.n_updates_per_iteration):
            ...
            vars = self.actor.trainable_variables + self.critic.trainable_variables
            grads = tape.gradient(total_loss , vars)
            self.optimizer.apply_gradients(zip(grads,vars))
            print(f" Loss = {total_loss.numpy():.6f},Adv: {tf.reduce_mean(advantages).numpy()}")
    ...
    def save(self):
        ...
        self.actor.save(self.models_dir + '/actor' , save_format='tf')
        self.critic.save(self.models_dir + '/critic',save_format='tf')
